...
```agda
module exercises.my-homework5v2 where
...
```agda
 ...
 ≼-unprime xs .(xs ++ zs) (zs , refl .(xs ++ zs)) = ≼-++ xs zs
```
...
```agda
 ...
 ≼-prime .(x :: xs) .(x :: ys) (::-is-prefix x xs ys xs≤ys) = ≼'-cons x xs ys (≼-prime xs ys xs≤ys)
```
...
```agda
 ≼-is-transitive : {X : Type} (xs ys zs : List X)
                 → xs ≼ ys → ys ≼ zs → xs ≼ zs
 ≼-is-transitive xs ys zs xs≤ys ys≤zs
  = ≼-unprime xs zs (≼'-is-transitive xs ys zs (≼-prime xs ys xs≤ys) (≼-prime ys zs ys≤zs))
```
