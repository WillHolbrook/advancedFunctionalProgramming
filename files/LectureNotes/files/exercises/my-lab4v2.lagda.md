...
```agda
module exercises.my-lab4v2 where
open import prelude
...
```agda
 ...
 ≤'-suc : (x y : ℕ) → x ≤' y → suc x ≤' suc y
 ≤'-suc x y (k , x+k=y) = k , ap suc x+k=y
 ...
 ≤'-pred x y (k , sx+k=sy) = k , ap pred sx+k=sy
```
...
```agda
 ...
 ≤-unprime zero y x≤'y = ≤-zero y
 ≤-unprime (suc x) (suc y) x≤'y = ≤-suc x y (≤-unprime x y (≤'-pred x y x≤'y))
```
...
```agda
 ...
 ≤-trans (suc x) (suc y) (suc z) (≤-suc .x .y x≤y) (≤-suc .y .z y≤z)
   = ≤-suc x z (≤-trans x y z x≤y y≤z)
...
Fin-is-exhaustively-searchable n = {!!}
```
