...
module my-insertion-sort-lecture (X : Type) where
...
record StrictTotalOrder (X : Type) : Type₁ where
  ...
  trichotomy x y = I (decidable x y)
    where
      I : is-decidable (x ≡ y) → (x < y) ∔ (x ≡ y) ∔ (y < x)
      I (inl x≡y) = inr (inl x≡y)
      I (inr ¬x≡y) = II (connected ¬x≡y)
        where
          II : (x < y) ∔ (y < x) → (x < y) ∔ (x ≡ y) ∔ (y < x)
          II (inl x<y) = inl x<y
...
module _ (τ : StrictTotalOrder X) where
 ...
 insert y [] = y :: []
 insert y (x :: xs) = I (trichotomy x y)
   where
     I : (x < y) ∔ (x ≡ y) ∔ (y < x) → List X
     I (inl x<y) = x :: insert y xs
     I (inr _) = y :: x :: xs
...
```agda
 ...
 insert-is-sorted y [] srtd = sing-sorted
 insert-is-sorted y (x :: []) sing-sorted = I (trichotomy x y)
  where
   I : (x < y) ∔ (x ≡ y) ∔ (y < x) → Sorted τ ((insert y (x :: [])))
   I (inl x<y) = {!!}
   I (inr or) = {!!}
 insert-is-sorted y (z :: (x :: xs)) (adj-sorted srtd x₁) = {!!}
 -- I (trichotomy x y)
 --  where
 --   I : (x < y) ∔ (x ≡ y) ∔ (y < x) → Sorted τ ((insert y (x :: xs)))
 --   I or = {!!}

 insert-all-is-sorted : (xs ys : List X) (ys-srtd : Sorted τ ys)
                      → Sorted τ (insert-all xs ys)
 insert-all-is-sorted [] ys ys-srtd = ys-srtd
 insert-all-is-sorted (x :: xs) ys ys-srtd = insert-is-sorted x (insert-all xs ys) IH
  where
   IH : Sorted τ (insert-all xs ys)
   IH = insert-all-is-sorted xs ys ys-srtd
