...
module my-natural-numbers-functions where
...
ap-≤₀-suc x y (diff , prf) = diff , ap suc prf

≤-to-≤₀ : (x y : ℕ) → x ≤ y → x ≤₀ y
≤-to-≤₀ zero y 0-smallest = y , refl y
≤-to-≤₀ (suc x) (suc y) (suc-preserves-≤ prf) = (fst IH) , (ap suc (snd IH))
  where
    IH : x ≤₀ y
    IH = ≤-to-≤₀ x y prf

≤₀-to-≤ : (x y : ℕ) → x ≤₀ y → x ≤ y
≤₀-to-≤ zero y prf = 0-smallest
≤₀-to-≤ (suc x) (suc y) (diff , prf) = suc-preserves-≤ (≤₀-to-≤ x y (diff , ap pred prf))
